...
store.counts.chain = fc.items[lvl==1,list(length(unique(IRI_KEY))),by = list(category,chain)]
store.counts = fc.items[lvl==1,list(length(unique(IRI_KEY))),by = list(category)]

chain.counts = fc.items[lvl==2,.N,by = list(category,UPC)]


store.counts = fc.items[lvl==1,.N,by = list(category,UPC,chain)]
...
chain.revenue.item = subset.summaries[lvl==1,list(chain.revenue=sum(revenue)),by = list(category,UPC,chain)]
