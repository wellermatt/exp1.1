...
f_calendar.daily_build = function()
{
	...
	calendar.daily = data.table(
		cal.date = as.Date(timeSequence(from = start.date, to = end.date, by = "day")), 
		WEEK = as.integer(rep(1:num.weeks,each=7)))
...
y = spw[fc.item=="00-01-18200-53030",UNITS]


library("tempdisagg"); library("zoo")
...
f_ts.agg.weeks.to.days = function(y)
{
	...
	z.daily = na.locf(y.daily, method = "constant", na.rm = FALSE)
	z.daily
}
...
f_calendar.holidays.us.get = function()
{
